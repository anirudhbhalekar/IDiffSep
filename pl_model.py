...
class DiffSepModel(pl.LightningModule):
    def __init__(self, config):
        ...
        self.validation_score_losses = []
        self.validation_separation_losses = []
    ...
    def validation_step(self, batch, batch_idx, dataset_i=0):
        ...
        self.validation_score_losses.append(loss.item())
        ...
        if self.trainer.testing or self.n_batches_est_done < self.valid_max_sep_batches:
            ...
            for name, loss in self.val_losses.items():
                sep_loss = loss(est, target)
                self.log(name, sep_loss, on_epoch=True, sync_dist=True)
                self.validation_separation_losses.append(sep_loss.item())

    def on_validation_epoch_end(self, outputs = None):

        output_dir = "/research/milsrg1/user_workspace/ab2810/IIB_Proj/diffusion-separation/validation_logs"
        os.makedirs(output_dir, exist_ok=True)
        score_loss_file = os.path.join(output_dir, f"{self.current_epoch}_validation_score_losses.json")
        separation_loss_file = os.path.join(output_dir, f"{self.current_epoch}_validation_separation_losses.json")

        # Save the score losses to a JSON file
        with open(score_loss_file, 'w') as f:
            json.dump(self.validation_score_losses, f)

        # Save the separation losses to a JSON file
        with open(separation_loss_file, 'w') as f:
            json.dump(self.validation_separation_losses, f)

        # Clear the lists
        self.validation_score_losses.clear()
        self.validation_separation_losses.clear()
